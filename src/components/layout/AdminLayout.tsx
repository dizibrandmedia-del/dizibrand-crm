...
export const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Scheduled Follow-ups', path: '/admin/followups', icon: Calendar },
    { label: 'Task Management', path: '/admin/tasks', icon: CheckSquare },
    { label: 'Sales & Invoices', path: '/admin/sales', icon: Award },
    { label: 'Analytics & Reports', path: '/admin/analytics', icon: BarChart3 },
    { label: 'Team Members', path: '/admin/team', icon: UserCheck },
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex antialiased transition-colors duration-200">
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col shrink-0 transition-transform duration-200 ease-in-out ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}
      >
        {/* Brand Header */}
        <div className="h-16 px-6 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          <Link to="/admin/dashboard" className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-extrabold text-sm tracking-tight text-slate-900 dark:text-white block leading-none">
                DIZIBRAND <span className="text-indigo-600 dark:text-indigo-400 font-semibold">CRM</span>
              </span>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-9 h-9 rounded-xl bg-indigo-600/20 border border-indigo-500/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-xs shrink-0">
                ...
                <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {unreadCount > 0 && (
                        <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border border-indigo-500/30">
                          {unreadCount} New
                        </span>
                      )}
                    </div>
